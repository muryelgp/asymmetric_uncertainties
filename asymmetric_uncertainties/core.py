...
class aufloat:
    ...
    def __init__(self, nv=None, err_n=None, err_p=None, confidence=None,
                 delta_chi2=None, data=[], mod=None):
        """
        ...
        err_n : float
            The negative error, the minus error (insert only the value, without
             the (-) sign).

        err_p : float
            The positive error, the plus error (insert only the value, without
            the (+) sign).

        confidence : int
            The statistical significance of the errors given by the percentile
            (%) confience intervals.
        ...
        delta_chi2: float
            The user can either define the error as a pre-defined confidence
            interval (confidence parameter) or to define it using a Δχ2 value
            Values tested range from 0.5 < delta_chi2 < 5.0.

        data: list or np.array
            Another way to Instantiate the class is to input a data list with
            a sampled PDF. Used to propagate uncertanties. See documetation
            for details.
        ...
        if not any(data):
            ...
            if(len(data) < 100):
                raise RuntimeError("'data' list needs to have more than 100\
                 elements for a minimuum relliability of the Monte Carlo\
                 simulation. At least 1000 elements is more advisible though")
        else:
            raise RuntimeError('Something went wrong! You have to either\
             insert the parameters (nm, err_n, err_p) or a list of values\
             (data). You can not insert both!')
        ...
        try:
            ...
            try:
                if delta_chi2 is not None:
                    ...
                    print('Confidence interval value needs to be chosen among 68, 90, 95, \
                        or you have to insert a Δχ2 value. Assuming 68% \
                        (Δχ2 = 1.0)!.')
                    confidence = 68
                    self.delta_chi2 = self._get_delta_chi2(confidence)
                    self._confidence = confidence
        ...
        if(str(self._creation_method) == 'by_parameters'):
            self._N = int(1e4)
            delta = 7 * np.max([self.err_n, self.err_p]) / np.sqrt(self.delta_chi2)
            ...
            self.delta_chi2_curve = np.asarray(
                self.delta_chi2_func(self.x_values))
            ...
            self.pdf_values = np.asarray(self.pdf(self.x_values))
            self.cdf_values = np.asarray(self._calculate_cdf_values())
            self.delta_chi2_curve = np.asarray(self.delta_chi2_func(
                self.x_values))
            self.data = np.asarray([])
    ...
    def __str__(self):
        if self._confidence is None:
            ...
            return output.format(
                self.nv, self.err_n, self.err_p, self.delta_chi2)
        else:
            output = "{:.2e} (-{:.2e}/+{:.2e})\n{:.0f}% confidence level \
            (Δχ2={:.2f})"
            return output.format(
                self.nv, self.err_n, self.err_p, self._confidence,
                self._get_delta_chi2(self._confidence))
    ...
    def pdf(self, x):
        ...
        (assumming the Variable Width Gaussian PDF, from R. Barlow's 2004 paper)
    ...
    def cdf(self, x):
        ...
        cdf = interpolate.interp1d(
            self.x_values, self.cdf_values, kind='nearest')
        return cdf(x)

    def _calculate_inverse_cdf(self, x):
        inverse_cdf = interpolate.interp1d(
            self.cdf_values, self.x_values, kind='nearest')
        return inverse_cdf(x)

    def gen_random(self):
        """
        Generates a random number using the probability density function of\
         the object
        (see .plot_pdf() function).
    ...
    def rescale_uncertainties(self, confidence=None, delta_chi2=None):
        ...
        if (confidence is None) & (delta_chi2 is None):
            raise RuntimeError("You have to select a 'confidence' value\
             or a Δχ2 (delta_chi2) value!")
        ...
        if confidence is not None:
            try:
                ...
                raise RuntimeError('Confidence interval value needs\
                 to be chosen among 68, 90 and 95')
        else:
            self.delta_chi2 = delta_chi2
    ...
    def _fit(self, expected=None, guess=None):
        ...
        flag = np.isfinite(self.data)
        y, x = np.histogram(self.data[flag], bins=50)
        ...
        x_scale = [1,
                   10**int(np.log10(abs(mod))),
                   10**int(np.log10(abs(((mod - min_data) * 0.4)))),
                   10**int(np.log10(abs(((max_data - mod) * 0.4)))),
                   1e-10]
        bounds = ([0, -np.inf, 0, 0, c],
                  [np.inf, np.inf, np.inf, np.inf, c + 1e-10])
        params, cov = curve_fit(self.fit_func,
                                x,
                                y / norm,
                                expected,
                                bounds=bounds,
    ...
    def plot_pdf(self, show=True, save=False):
        ...
        if self._confidence is None:
            ...
            plt.plot(self.x_values, self.pdf_values, color="blue",
                     label=output.format(self.nv, self.err_n,
                                         self.err_p, self.delta_chi2))
        else:
            output = "{:.2e} (-{:.2e}/+{:.2e})\n{:.0f}% confidence level (Δχ2={:.2f})"
            plt.plot(self.x_values, self.pdf_values, color="blue",
                     label=output.format(self.nv, self.err_n, self.err_p,
                                         self._confidence,
                                         self._get_delta_chi2(self._confidence)))

        ymin, ymax = plt.ylim()
        plt.ylim(ymin, 1.4 * ymax)
        plt.vlines(self.nv + self.err_p, ymin=0,
                   ymax=self.pdf(self.nv + self.err_p),
                   color='black', linestyle='--', linewidth=0.8)
        plt.vlines(self.nv - self.err_n, ymin=0,
                   ymax=self.pdf(self.nv - self.err_n),
                   color='black', linestyle='--', linewidth=0.8)
        plt.vlines(self.nv, ymin=0,
                   ymax=self.pdf(self.nv),
                   color='black',
                   linestyle='--',
                   linewidth=1.5)
